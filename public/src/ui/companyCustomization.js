...
export function installCompanyCustomization(store, root = document.getElementById("app")) {
  ...
  root.addEventListener("click", (event) => {
    ...
    if (resetPrice) {
      if (Number.isFinite(Number(current.analysisPrice))) updateReportPrice(store, ticker, Number(current.analysisPrice));
...
export function readCompanyCustomization(ticker) { return readCustom()[String(ticker || "").trim().toUpperCase()] || null; }
...
function mountPanel(store, root) {
  ...
  if (social) social.insertAdjacentElement("afterend", section); else host.prepend(section);
}

function reapplyStoredCustomizations(store) { const custom = readCustom(); for (const [ticker, value] of Object.entries(custom)) applyCustomizationToReports(store, ticker, value, { silent: true }); }
function applyCustomizationToReports(store, ticker, custom, options = {}) {
  const list = store.state.externalAnalyses?.[ticker]; if (!Array.isArray(list) || !list.length) return; let changed = false;
  const nextList = list.map((report) => { const next = structuredCloneSafe(report);
    if (custom.logoDataUrl && next.company?.logoUrl !== custom.logoDataUrl) { next.company = { ...(next.company || {}), logoUrl: custom.logoDataUrl }; changed = true; }
    if (!custom.logoDataUrl && next.company?.logoUrl) { next.company = { ...(next.company || {}) }; delete next.company.logoUrl; changed = true; }
    if (Number.isFinite(Number(custom.currentPrice)) && Number(next.fairValueSummary?.currentPrice) !== Number(custom.currentPrice)) { applyPriceToReport(next, Number(custom.currentPrice)); changed = true; }
    return next; });
  if (changed && !options.silent) store.set({ externalAnalyses: { ...(store.state.externalAnalyses || {}), [ticker]: nextList } }); else if (changed && options.silent) store.state.externalAnalyses = { ...(store.state.externalAnalyses || {}), [ticker]: nextList };
}
function updateReportPrice(store, ticker, price) { const list=store.state.externalAnalyses?.[ticker]; if(!Array.isArray(list))return; const nextList=list.map((report)=>{const next=structuredCloneSafe(report);applyPriceToReport(next,price);return next;}); store.set({externalAnalyses:{...(store.state.externalAnalyses||{}),[ticker]:nextList}}); }
function applyPriceToReport(report, price) { const base=Number(report.fairValueSummary?.fairValueBase); const fair={...(report.fairValueSummary||{}),currentPrice:price}; fair.upsideDownsidePercent=Number.isFinite(base)&&price>0?(base/price-1)*100:null; fair.marginOfSafetyPercent=Number.isFinite(base)&&base!==0?((base-price)/base)*100:null; report.fairValueSummary=fair; }
function paintLogos(store,root){const custom=readCustom();root.querySelectorAll(".library-company-card[data-external-ticker]").forEach((card)=>paintLogo(card.querySelector(".company-logo"),custom[String(card.dataset.externalTicker||"").toUpperCase()]?.logoDataUrl));const ticker=selectedTicker(store);if(ticker&&custom[ticker]?.logoDataUrl)root.querySelectorAll(".report-app-bar .company-logo").forEach((node)=>paintLogo(node,custom[ticker].logoDataUrl));}
function paintLogo(node,dataUrl){if(!node||!dataUrl)return;let image=node.querySelector("img");if(!image){image=document.createElement("img");image.alt="";node.prepend(image);}if(image.src!==dataUrl)image.src=dataUrl;image.hidden=false;node.querySelector(".company-logo-fallback")?.setAttribute("hidden","");node.classList.add("has-image");node.classList.remove("is-fallback");}
async function compressLogo(file){const bitmap=await createImageBitmap(file);const max=384;const scale=Math.min(1,max/Math.max(bitmap.width,bitmap.height));const canvas=document.createElement("canvas");canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));const ctx=canvas.getContext("2d");ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality="high";ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close?.();return canvas.toDataURL("image/png");}
function selectedTicker(store){return String(store.state.externalReportSelection?.ticker||"").trim().toUpperCase();}
function selectedReport(store){const ticker=selectedTicker(store);const id=store.state.externalReportSelection?.reportId;const list=store.state.externalAnalyses?.[ticker]||[];return list.find((item)=>item?.id===id)||list[0]||null;}
