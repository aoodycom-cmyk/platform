...
- Added deterministic requirement achievement calculation where `PASSED` and `EXCEEDED` receive full weight, `PARTIALLY_PASSED` receives half weight, `FAILED` receives zero, and `NOT_REPORTED` is excluded from the denominator.
- Added report sections for Guidance, dynamic company KPIs, requirements to justify the next target price, earnings requirement results, valuation method used, and historical changes.
- Added local-first Export All Investment Data and Restore Investment Data with validation, preview, merge/replace options, and secret-key exclusion.
- Added a DEMO external report with fake data for testing Guidance, KPIs, price target requirements, earnings results, valuation method selection, and history behavior.
- Added tests for new external report fields, requirement calculations, historical requirement immutability, backup export/restore, all six external recommendation values, and UI/style coverage.
- Preserved the existing External ChatGPT workflow, saved analysis compatibility, local-first storage, and internal Franklin investment engines.
- No OpenAI API integration, backend AI call, subscription flow, or automatic financial analysis was added.
...
- Tightened mobile Institutional Research cards so nested profile cards stay full-width in RTL and use calmer typography.
