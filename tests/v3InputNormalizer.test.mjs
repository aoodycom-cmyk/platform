...
const raw = {
  ...
  forecast: {
    ...
    yearlyForecast: [{ revenue: { basis: "analyst assumption" } }],
    changedAssumptions: [{ direction: "up" }]
  },
  nextRequirements: {
    mode: "advance target",
...
assert.equal(normalized.forecast.yearlyForecast[0].revenue.basis, "analyst_assumption");
assert.equal(normalized.forecast.changedAssumptions[0].direction, "UP");
