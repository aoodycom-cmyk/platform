...
const tabs = [['summary','summary'],['earnings','earnings'],['company','company'],['strengths','strengths-risks']];
const report = { viewport:{width:390,height:844}, pages:{}, consoleErrors };

for (const [tab,key] of tabs) {
  await page.evaluate((tab) => {
    const store = window.__equityResearchStore;
    const sel = store.state.externalReportSelection || {};
    const score = store.state.quarterlyScorecard || {};
    const ticker = sel.ticker || score.originTicker || score.ticker;
    const reportId = sel.reportId || score.originReportId || 'latest';
  ...
  report.pages[key] = await page.evaluate(() => {
    ...
    const css = el => { if(!el) return null; const s=getComputedStyle(el); return {fontSize:s.fontSize,fontWeight:s.fontWeight,color:s.color,backgroundColor:s.backgroundColor,padding:s.padding}; };
    ...
    return {
      ...
      titleSamples:[...document.querySelectorAll('h1,h2,h3')].slice(0,10).map(el=>({text:el.textContent.trim().slice(0,90),fontSize:getComputedStyle(el).fontSize,rect:rect(el)}))
    };
  });
}
